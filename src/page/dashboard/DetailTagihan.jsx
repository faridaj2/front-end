...
function DetailTagihan() {
            ...
            .then((res) => {
                ...
                if (parseInt(data.bulanan) === 1 && data.meta) {
                    ...
                    const mergedArray = array1.map((month) => {
                        const price = mapArray2[month]
                            ? mapArray2[month].price
                            : data.default;
                        return { month, price };
                    });
    ...
    return (
        <DashboardTemplate>
            <Scroll>
                <div className="text-white flex gap-2 pt-2">
                    Detail Tagihan{" "}
                    <span className="truncate w-1/2 inline-block">
                        {data && data.name}
                    </span>
                </div>
                ...
                {data ? (
                    ...
                    data.arrayMonth.map((item) => (
                        <div key={item.month} className="mb-3 mr-1">
                            <div className="bg-brown text-white shadow-md p-4 rounded-3xl">
                                <div className="flex justify-between items-center">
                                    <Chip className="bg-dark text-white">
                                        {item.month}
                                    </Chip>
                                    <Chip
                                        color={`${getStatus(item.month, item.price) == "Lunas" ? "primary" : "danger"}`}
                                        variant="shadow"
                                        className="text-tiny"
                                    >
                                        {getStatus(item.month, item.price)}
                                    </Chip>
                                </div>
                                <Divider className="my-2" />
                    ...
                    <div key={1} className="mb-3">
                        <div className="bg-white shadow-md p-4 rounded-3xl border-1">
                            <div className="flex justify-between items-center">
                                ...
                                <Chip
                                    color={`${getStatus(data.tenggat, data.default) == "Lunas" ? "primary" : "danger"}`}
                                    variant="shadow"
                                    className="text-tiny"
                                >
                                    {getStatus(data.tenggat, data.default)}
                                </Chip>
                            </div>
                            <Divider className="my-2" />
                            <div className="flex justify-between w-full">
